...
class MyApp extends StatelessWidget { //MyAppクラスはFlutterアプリのルートウィジェット(ウィジェットツリーの最上位)
  const MyApp({super.key}); //ウィジェットの一意識別に必要らしいがなくても動く(調べても難しかった)

  @override //メソッド前にいつもつけてる。buildメソッドはStatelessウィジェットだよって感じ
  Widget build(BuildContext context) { //buildメソッドはFlutterがこのウィジェットを画面に描画する際に呼び出すメソッド。
                                       //BuildContextはbuildメソッドの引数によくいるやつ
    return MaterialApp( //インポートしてきた便利なやつ。これをreturnすることで中にあるものを描写される
